...
if (!class_exists('CaptchaDebugLog')) {
	class CaptchaDebugLog
	{
		...
		{
			if (!self::$instance) {
				self::$instance = new CaptchaDebugLog();

				self::$instance->_file_ext = 'log';
				self::$instance->_log_path = TENCENT_WORDPRESS_CAPTCHA_LOGS;
		...
		{
			self::_init();
			$tencent_wordpress_captcha_options = get_option('tencent_wordpress_captcha_options');
			$loginNeedCode = is_array($tencent_wordpress_captcha_options) && isset($tencent_wordpress_captcha_options['debug_need_code'])
				? $tencent_wordpress_captcha_options['debug_need_code']
				: '1';
			if ($loginNeedCode == '2') {
