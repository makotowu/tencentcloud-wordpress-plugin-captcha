...
jQuery(function ($) {
	...
	$('#tencnetcloud-codeVerify-setting-update-button').click(function () {
		...
		var aidEncryptedOn = $("#codeVerify-option-aidEncrypted").is(":checked") ? 2 : 1;;
		var aidEncryptedExpire = $("#codeVerify-option-aidEncryptedExpire").val();
		...
		$.ajax({
			...
			data: {
				...
				aidEncryptedExpire: aidEncryptedExpire,
			...
			success: function (response) {
				var msg = response && response.data && response.data.msg ? response.data.msg : (response && response.success ? '保存成功' : '保存失败');
				showAjaxReturnMsg(msg, !!(response && response.success));
			...
			error: function () {
				showAjaxReturnMsg('保存失败，请稍后重试', false);
				btn.data('saving', 0);
				btn.prop('disabled', false);
				btn.text(oldText);
			}
